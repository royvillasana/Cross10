...
- [x] 0.5 Confirm `npm run dev` serves the neutral starter, then switch `src/app/app-acceptance-data.ts` to `mode: "product"` with `productName`, `productSummary`, `requestedBehavior` — must land with 2.x and its acceptance rows as one coherent batch **Audited (4.1): done.** Verified: `app-acceptance-data.ts:120` is `mode: "product"` with name and summary.
- [x] 0.6 Declare `exportIntent` as image and video only, with a written note that shader source is **not** an artifact and leaves through clipboard or MCP (R55) **Audited (4.1): done.** Verified: `exportIntent` declares image and video, with the note that shader source is not an artifact (R55).
- [x] 0.7 Declare `viewInteraction: "non-spatial"` with the reason Croix10 recorded — 2D shader output, no scene geometry to orbit **Audited (4.1): done.** Verified: `viewInteraction` is declared at `app-acceptance-data.ts:126`.
- [x] 0.8 Declare `interactionOwnership` assigning layer management to **Layers**, so no `fileDrop` row restates reorder or transform claims the layer recipes already prove (`component-contracts.media-custom.ts:72`) **Audited (4.1): done.** Verified: `interactionOwnership` is declared at `app-acceptance-data.ts:58`.
- [x] 0.9 Author the Control Selection Inventory for the layer surface first: for each setting record need, value model, built-ins checked, chosen built-in, why, rejected alternatives, target, renderer mapping, acceptance coverage **Audited (4.1): done.** Verified by its output: the layer surface's controls each carry need, value model and built-in choice through the section files.
- [x] 0.10 Author the entity decomposition and populate `appControlSectionInventory` — every section titled by the entity it edits, every gate in the entity it gates, none over ten controls **Audited (4.1): done.** Verified: eighteen inventory entries, each with `entityId`.
- [x] 0.11 Write the Animation Intent Inventory selecting the keyframes timeline, with the derived loop period for `panels.timeline.defaultDurationSeconds` **Audited (4.1): done.** Done later and differently. The intent selects **playback**, not keyframes, with a product-derived six-second period — see `video-export-and-motion`.
- [x] 0.12 Write the Renderer Technique Decision Matrix, arguing WebGL2 over a Canvas 2D baseline rather than assuming it **Audited (4.1): done.** Verified: the matrix argues WebGL2 against Canvas 2D and WebGPU in `app-performance.ts`.
- [x] 0.13 Declare `workloadEnvelope` dimensions — layer **count** is a new workload dimension Croix10 never had, since the assembled program grows with the stack **Audited (4.1): done.** Verified: `stack-depth` is declared as a runtime-state workload dimension the schema has no control for.
- [x] 0.14 Compile `rendererPipelineRegistration` for the stack-composed pass chain, with cache keys that include the stack signature **Audited (4.1): done.** Verified: `studio-pipeline.ts` registers the pass chain with cache keys.
- [x] 0.15 Run `assessToolcraftRenderPlan` and resolve every structural error **Audited (4.1): done.** Verified: `assessToolcraftRenderPlan` runs in the performance gates and its errors are resolved.
- [x] 0.16 Switch `docs/toolcraft/agent-worklog.md` to `Mode: product` with the first Decision Trail entry and all eight decision sections **Audited (4.1): done.** Verified: the worklog is `Mode: product` with a Decision Trail.
- [ ] 0.17 Run `npm run ai:check` **Audited (4.1): still open, and now measured.** `npm run ai:check` fails with fifteen findings — thirteen files over their line budget and two `any` usages. All fifteen predate this session (confirmed against `d82fc74`); the two this session added were fixed rather than left.
...
- [x] 1.1 Move the GLSL chunk registry and variant assembler with its variant cache → `studio-shaders.ts` **Audited (4.1): superseded.** The named module was never created; `studio-layers.ts` holds the chunk registry and assembler.
- [x] 1.2 Move the ramp chunk — coordinate, sample, proximity push, slot colour → `studio-shaders-ramp.ts` **Audited (4.1): superseded.** The named module was never created; the ramp chunk lives in the gradient body.
- [x] 1.3 Move the schema-derived uniform upload, including sRGB-to-linear conversion for colour types → `studio-render.ts` **Audited (4.1): superseded.** The named module was never created; uniform upload with sRGB-to-linear lives in `studio-stack-render.ts` and `studio-scene.ts`.
- [x] 1.4a Move the parameters module → `studio-parameters.ts`, and the pipeline pass → `studio-pipeline.ts` **Audited (4.1): superseded.** The named module was never created; parameters became `studio-scene.ts`; the pass is `studio-pipeline.ts`.
- [x] 1.4b **Deferred to group 2.** The scene reader is the one moved module that reads schema state, and this app has no layer schema yet. Moving it verbatim would import Croix10's engine-shaped state model into an app whose state is a layer stack, so it is rewritten in 2.x against the real schema rather than copied and then undone **Audited (4.1): superseded.** The named module was never created; the scene reader is `studio-scene.ts`, written against the layer model rather than moved verbatim.
- [x] 1.5 Move the timeline wiring: loop time from `getToolcraftTimelineLoopTime`, no wall-clock read anywhere in the render path — lands with 1.4b, since it reads timeline state through the same reader **Audited (4.1): superseded.** The named module was never created; loop time comes from `getToolcraftTimelineLoopProgress`, with no wall-clock read in the render path.
...
- [x] 2.5a Land **four** runtime acceptance rows — `selection`, `visibility`, `reorder`, `grouping` — each with automated *and* browser proof and a written observable. Grouping is obliged by `runtime-coverage.ts:21` whether or not the product wanted it, so it ships as a real feature rather than a stub (R50) **Audited (4.1): done.** Verified: all four runtime rows exist with automated and browser proofs.
...
- [x] 2.8 Acceptance rows for the layer surface, every `selectedLayer.*` control declaring `layerCoverage: "selected-layer-controls"` alongside its own coverage (R51): reordering changes what covers what; a per-layer edit changes only that layer **Audited (4.1): done.** Verified: thirty-three rows carry `layerCoverage: "selected-layer-controls"`.
...
- [x] 15.2 **A rotation handle on the canvas.** The grip is a node in the point list 14.3 introduced: it sits off the shape's north edge, turns with it, and writes `selectedLayer.maskRotation` through the same `shape-shaping` entry (`controls.setValue`) the other gestures answer to. The note it carried was real and is fixed with it — the extent nodes were placed from the axis-aligned screen rectangle, so on a turned shape they pointed at pixels the shape does not occupy. Placement *and* the resize arithmetic now work in the shape's own frame, which is the frame the mask tests against; at rest the two frames coincide, so the existing proofs read unchanged
- [x] 15.3 **Retired `Shape rotation` from the sidebar**, for the reason 14.1 retired the other four. The control row that read the slider went with it and the canvas-handle row stands in its place, so the turn is still proved — where the turning now happens. **Carried a trap worth remembering**: the readers those browser proofs use are serialized into the page, so removing the `sliderValue` helper beside the rotation reading left a *third* reader calling a name TypeScript still resolved against an enclosing scope. It typechecks and is undefined in the browser; four treatment proofs failed. Repaired in the 15.4 commit
- [x] 15.4 **Keyboard shortcuts, after the ownership audit.** Confirmed in the code rather than assumed: `ToolcraftRoot` already listens on the document for Cmd/Ctrl+Z, Shift+Z and Ctrl+Y, and the toolbar contract says in as many words not to add app-level ones; `canvas.zoomIn/zoomOut/zoomReset/center` are toolbar commands over a viewport the shell draws; a V select tool would be a mode for a tool this product has no concept of, since selection is the runtime panel's by the `layer-selection` entry. So the product takes exactly one key, P for the pen, and `studio-shortcuts.ts` records the three refusals beside it. **No `interactionOwnership` entry, and this is the finding**: that model's surfaces are `canvas | panel` only, and a shortcut is neither — it is an accelerator for an operation that already has an owning surface. Both the Draw button and P run `planStudioPenDrawing`, which is what keeps one operation from becoming two implementations. It carries its own acceptance row (`stack.pen.shortcut`, kind `runtime`) so the key is checked rather than merely implied
...
- [x] 4.1 **The stripe field controls are all carried now.** Mirror, band separator and jitter amount landed earlier; the last of them lands here, and it is not the control the task named
  - **The section budget that blocked this is gone.** It was a limit on the *entity*: `Selected layer` stood at ten controls in one section and an eleventh failed. Splitting the entity into workflow stages (13.x/14.x) moved the limit to each stage, and `Layer Pattern` stood at nine of ten. It stands at ten now — so the next per-layer pattern control needs another stage before it can land, and that is a real constraint to plan around rather than a surprise to hit
  - **`Jitter frequency` does not exist here, and could not.** Croix10's *Wobble rate* scaled a noise coordinate *along* the band, so the rate at which a band wobbled down its own length was a real, continuous thing to set. This field's jitter is per band *index* — which is what keeps bands as bands rather than dissolving them into noise — and the index goes through a hash, so neighbouring bands never correlate at any value. There is no rate left to control. What remains is *which* arrangement of displacements the field draws, so the control is **Jitter variation**, stepped whole numbers because each position is a different arrangement rather than more of anything, and the uniform is renamed from `jitterFrequency` to `jitterVariation` so the delivered source says what it means (R65's rule: a control the product cannot keep true is worse than one it does not offer)
  - **The 14.x deferral is resolved, not overridden.** That decision was recorded in the code: no control, because the only observable is the arrangement itself and asserting one would pin a fingerprint that depends on backing size and GPU. It was right about the statistics — count, balance and run spread all hold steady across the domain — and the way past it is a *differential* claim rather than a fingerprint. At six bands with the jitter at nine tenths, three places inside the shape each swap ink between variation 12 and variation 5, while the count, the amount and both inks stay exactly where they were. Measured before the expectation was written, and stable across repeated runs
- [x] 4.2 **The chromatic ramp is carried, as the layer's own controls rather than as a compound gradient (R70).** Three of the four parts were already here and are not siblings *of* anything: the ramp's type is `Transition shape`, its angle is the layer's own, and its stops are the palette slots. What was missing is where the ramp starts, which is Croix10's 9A.3
  - It lands as `Offset`, the control that already means this: a band sequence slides along its axis and a ramp along its own, so one control covers both kinds and the gradient body reads a `phase` uniform named for the stripes' own. The domain becomes signed, because a band sequence repeats and a ramp does not
  - **The section rules reached the same answer independently.** A control gated by `selectedLayer.type` must sit in the section holding that gate (R34) and `Layer Pattern` is full at ten, so a gradient-only offset had nowhere to live. The shared one needed no new place
  - **Not declaring the compound `gradient` control is the decision, not an omission.** It would restate the angle, the transition shape and the palette as fields of a second control — three operations with two surfaces each, which is what 14.1 and 15.3 exist to remove. R23 forbids splitting a compound control's fields into siblings; it does not oblige a product whose surface was never built that way to assemble one
  - **Stop positions and stop opacity stay undelivered, and R70 records the price.** Uneven stops are a position per slot, which is a collection and therefore the compound control's job; per-stop opacity is a second alpha inside a layer that already has one. Neither is needed by the reference works
- [x] 4.3 **The cursor field is carried, and every clause of this task is already satisfied by 14.6/14.7 (R68).** Checked rather than assumed, one clause at a time: it is a *per-layer* effect (`Follow the pointer` is a per-layer switch, opt-in, with its own row and browser proof — a stack where every layer chased the pointer would be a toy); the hotspot is *committed to state* (`stack.cursor`, an uncontrolled value target written by the canvas and read by the scene builder, which is what lets the export and the delivered source bake the position the author left it at); and it is *persisted through `additionalValueTargets`*, which `app-schema.ts` declares beside the layer record
  - Verified in a real browser rather than read off the schema: with an induction engine following the pointer, the committed position appears in `localStorage` under `stack.cursor`, and a reload restores a frame that still carries the induced fringe where the pointer had been
  - **The reload reading has a confound worth stating**, since the reason it was not turned into a proof: a reload does not move the physical pointer, so the app re-commits a position of its own shortly after, and at the pointer's resting place the restored value and the re-committed one name the same place. Separating them needs a fixture that can reload with the pointer somewhere the restored value is not, which is a proof about the runtime's pointer path more than about this product
  - **No radius or falloff controls, deliberately (R68).** Croix10's 9B declared four: enable, radius, falloff, strength. Here the cursor reaches the engines as one quantity — how near the pointer is, across a fixed radius — and each technique multiplies its own amount by it, so strength is the engine's amount and the other two would be settings for a shape nobody is choosing
- [x] 4.4 Carry the palette and colour slot surface (delivered as 12.0)
- [x] 4.5 **Acceptance rows and browser proofs, batched with each control rather than after it.** Every control carried in this group landed with its row, its automated test and its browser proof in the same commit — `selectedLayer.jitterVariation` with the differential arrangement reading, `selectedLayer.phase` re-proved over both field kinds in one test when it stopped being stripes-only. The batching rule held on its own terms: nothing here needed a follow-up pass to acquire its proof
- [ ] 4.6 Run `npm run verify:delivery` — same gate as 2.10, blocked on the same authorization. Not a product gap

## 5. Gallery

- [x] 5.1 **The gallery is a picker and an apply (R71)**, applied through runtime commands. Applying is a stack replacement — `layers.delete` per existing layer, `layers.add` per preset layer with explicit draft ids, one write of `stack.layerRecord` for exactly those ids, then `layers.select` on the topmost. The plan is a pure function, so what makes it a replacement rather than a merge is testable without a runtime
- [x] 5.2 **Ten compositions, each expressed as a stack.** Croix10's library has ten entries, not the eleven this task assumed. Six were named for engines that did not exist here when R58 was written; group 4 landing is what made them expressible, and three of them turned out to be the stack's own model rather than an engine (R67) — `Additive Bands` is a band field over a support, because what shows through a separator in a stack is the layer beneath, and `Transchromie Sheets` is three multiplying layers
  - Values are authored as a control holds them — hex for a colour, the option's own value for a select — and converted once by the same function that folds a control edit into the record. Authoring them in the shader's units would mean hand-written linear triples and option indices
  - Every layer sets `maskSize: 0`. A layer arrives confined to a shape (R65), which is right for building a composition and wrong for a preset: these are whole-frame fields
