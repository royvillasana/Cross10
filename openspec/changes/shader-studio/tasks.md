...
- [x] 14.4 **Pen tool**, split where the seam is real. A pen with nothing to store into and a `free` form with nothing able to author it are each unprovable alone; a path that exists in state and is drawn on the overlay is testable end to end today.
  ...
  - **Owed — the workload dimension.** The crossing test walks the path once per pixel, so per-layer cost now varies with a length the author chooses. The growth class is unchanged but the per-layer constant is author-controlled, which is what a dimension declares. Declaring it needs a fixture adapter that can build a path of a given length — the pen driven from the performance fixture. The pipeline comment names the gap rather than staying silent
