...
- [~] 14.4 **Pen tool**, split where the seam is real. A pen with nothing to store into and a `free` form with nothing able to author it are each unprovable alone; a path that exists in state and is drawn on the overlay is testable end to end today.
