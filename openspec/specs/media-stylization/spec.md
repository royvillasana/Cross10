...
2026-08-18 (`outstanding` 1.1). A picture can be imported, placed, flipped,
turned and treated. What is *not* built is the stylization this file is named
for: nothing re-renders a source through the engines, and nothing about video
exists at all.
## Requirements
...
**Status: satisfied, with one clause now deliberately broken.** Import is the
runtime's `fileDrop`, there is no upload UI on the canvas, and no file list or
sorting was built.

"No custom upload button" is the clause that gave way. The product owner asked
three times for import to sit beside "add a layer", where the runtime's add menu
takes no product items (upstream issue 18), so the product adds a **Media** entry
to that menu which presses the runtime's own file input. It is a relay rather
than a second import path — the runtime still reads the file, allocates the
asset and creates the layer. The drop zone itself is hidden while empty and
returns with the picture, because the transforms beside it *are* panel work.
...
**Status: satisfied.** `renderDefaultCanvasMedia: false`, for the reason given
here.
...
**Status: satisfied.** An import never resizes the canvas, and the Setup
controls stay. The picture is drawn inside its layer's region rather than
cover-cropped to the canvas — which is the same guarantee arrived at from the
other side: a layer is a shape, so a picture is bounded by the shape it is in.
...
**Status: satisfied.** No webcam, no `MediaStream`, and the reasoning still
holds — a live feed cannot be re-rendered at a scheduled export time, which this
product now depends on for every video frame it writes.
