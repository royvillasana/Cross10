...
describe("planStudioTechniqueChange", () => {
  const preset = STUDIO_PRESETS[1]!;
  const base = {
    layers: STACK,
    preset,
    record: {} as StudioLayerRecord,
    selectedLayerId: "ground",
  };

  it("offers rather than acts when there is work to replace", () => {
    const commands = planStudioTechniqueChange({
      ...base,
      confirmed: false,
    ...
    expect(commands).toHaveLength(1);
    expect(commands[0]?.target).toBe(STUDIO_PENDING_TECHNIQUE_TARGET);
    expect(commands[0]?.value).toBe(preset.id);
  });

  it("leaves the canvas untouched while the offer stands", () => {
    const commands = planStudioTechniqueChange({
      ...base,
      confirmed: false,
      pendingTechnique: null,
  ...
  it("applies without asking when there is no work to replace", () => {
    const commands = planStudioTechniqueChange({
      ...base,
      confirmed: false,
      layers: [],
    ...
    expect(commands.some((command) => command.type === "layers.add")).toBe(true);
    expect(commands.some((command) => command.target === STUDIO_TECHNIQUE_TARGET)).toBe(
      true,
    );
  });

  it("captures no snapshot for an empty canvas, since there is nothing to restore", () => {
    const commands = planStudioTechniqueChange({
      ...base,
      confirmed: false,
      layers: [],
    ...
    expect(commands.some((command) => command.target === STUDIO_SNAPSHOT_TARGET)).toBe(
      false,
    );
  });

  it("carries the change out once it has been agreed to", () => {
    const commands = planStudioTechniqueChange({
      ...base,
      confirmed: true,
      pendingTechnique: preset.id,
    });

    expect(commands.some((command) => command.type === "layers.delete")).toBe(true);
    expect(commands.some((command) => command.type === "layers.add")).toBe(true);
  });

  it("keeps a confirmed change revertible", () => {
    // Agreeing to proceed is not agreeing to lose the work, so the snapshot is
    // captured on the confirming press exactly as it would be without one.
    const commands = planStudioTechniqueChange({
      ...base,
      confirmed: true,
      pendingTechnique: preset.id,
    ...
    const snapshot = commands.findIndex(
      (command) => command.target === STUDIO_SNAPSHOT_TARGET,
    );
    const firstDelete = commands.findIndex((command) => command.type === "layers.delete");
    expect(snapshot).toBeGreaterThanOrEqual(0);
    expect(snapshot).toBeLessThan(firstDelete);
  });

  it("clears the offer and records the technique once it has run", () => {
    const commands = planStudioTechniqueChange({
      ...base,
      confirmed: true,
      pendingTechnique: preset.id,
    ...
    expect(
      commands.find((command) => command.target === STUDIO_PENDING_TECHNIQUE_TARGET)
        ?.value,
    ).toBe("");
    expect(
      commands.find((command) => command.target === STUDIO_TECHNIQUE_TARGET)?.value,
    ).toBe(preset.id);
  });

  it("refuses a confirmation for an entry that was never offered", () => {
    // Browsing the thumbnails after arming and then confirming must apply what
    // was offered, not whatever is under the cursor now.
    expect(
      planStudioTechniqueChange({
        ...base,
        confirmed: true,
        pendingTechnique: STUDIO_PRESETS[0]!.id,
      }),
    ).toEqual([]);
  });

  it("refuses a confirmation when nothing was offered at all", () => {
    expect(
      planStudioTechniqueChange({ ...base, confirmed: true, pendingTechnique: null }),
    ).toEqual([]);
  });

  it("re-offering the same entry carries it out, which is the second press", () => {
    const commands = planStudioTechniqueChange({
      ...base,
      confirmed: false,
      pendingTechnique: preset.id,
...
describe("planStudioTechniqueDecline", () => {
  it("writes exactly one thing and touches nothing else", () => {
    const commands = planStudioTechniqueDecline();
  ...
  it("leaves the technique the canvas is in alone", () => {
    // "The previously chosen technique is still the current one" is only true
    // if declining never writes to it.
    expect(
      planStudioTechniqueDecline().some(
        (command) => command.target === STUDIO_TECHNIQUE_TARGET,
      ),
    ).toBe(false);
  });
});

describe("the chromatic engine over a picture", () => {
  it("gives the image kind the same engine uniforms every other kind has", () => {
    const image = new Map(
      studioLayerUniforms("image").map((uniform) => [uniform.name, uniform] as const),
    );
    const stripes = new Map(
      studioLayerUniforms("stripes").map((uniform) => [uniform.name, uniform] as const),
    );

    for (const name of ["engine", "engineAmount", "engineCursor", "enginePitch"]) {
      expect(image.has(name)).toBe(true);
      // Same reading of the same value, or an entry applied to a picture would
      // carry an engine that meant something else once it got there.
      expect(image.get(name)?.optionValues).toEqual(stripes.get(name)?.optionValues);
      expect(image.get(name)?.defaultValue).toEqual(stripes.get(name)?.defaultValue);
    }
  });

  it("declares the engine before the sampler, which the body's signature reads in order", () => {
    // The argument list is the uniform list, so a uniform declared out of order
    // arrives silently in the wrong parameter -- which this project has already
    // spent three diagnoses on once.
    const names = studioLayerUniforms("image").map((uniform) => uniform.name);
    expect(names.indexOf("engine")).toBeLessThan(names.indexOf("enginePitch"));
    expect(names.indexOf("enginePitch")).toBeLessThan(names.indexOf("image"));
  });

  it("leaves the picture untouched at the engine's identity", () => {
    // The body returns the sample directly below the branch threshold, which is
    // what makes an untreated picture cost nothing and look like itself.
    const image = new Map(
      studioLayerUniforms("image").map((uniform) => [uniform.name, uniform] as const),
    );
    expect(image.get("engine")?.defaultValue).toBe(0);
  });
});
