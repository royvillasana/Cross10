...
export const studioHookAcceptanceRows: readonly ToolcraftComponentAcceptance[] = [
  ...
  {
    automated: true,
    automatedTestName: "declares a hook parameter drives the frame from the author's own code",
    browser: true,
    browserTestName: "browser: studio hook parameter drives the frame from the author's code",
    componentType: "slider",
    evidence: "rendered-pixels",
    expectedObservable:
      "Your parameter A drives whatever the author's chunk reads it for: moving it changes the frame, and the change is whatever the code says it is rather than anything the schema decided.",
    fixture: SELECTED_LAYER_FIXTURE,
    id: "stack.hookA",
    kind: "control",
    target: "stack.hookA",
    userAction: "Write a chunk that reads uHookA, then move Your parameter A.",
  },
  {
    automated: true,
    automatedTestName: "declares a hook parameter is independent of the other knobs",
    browser: true,
    browserTestName: "browser: studio hook parameters move independently of one another",
    componentType: "slider",
    evidence: "rendered-pixels",
    expectedObservable:
      "Each knob is its own value: moving Your parameter B changes the frame without moving what the other three carry, so a chunk reading two of them gets two answers rather than one.",
    fixture: SELECTED_LAYER_FIXTURE,
    id: "stack.hookB",
    kind: "control",
    target: "stack.hookB",
    userAction: "Write a chunk that reads uHookB, then move Your parameter B.",
  },
  {
    automated: true,
    automatedTestName: "declares a hook parameter survives a reload with the composition",
    browser: true,
    browserTestName: "browser: studio hook parameters survive a reload with the composition",
    componentType: "slider",
    evidence: "rendered-pixels",
    expectedObservable:
      "A knob's value is part of the composition and is persisted with it, so reloading returns the frame the author left rather than a chunk reading zeros.",
    fixture: SELECTED_LAYER_FIXTURE,
    id: "stack.hookC",
    kind: "control",
    target: "stack.hookC",
    userAction: "Write a chunk that reads uHookC, then move Your parameter C.",
  },
  {
    automated: true,
    automatedTestName: "declares a hook parameter is baked into the delivered source",
    browser: true,
    browserTestName: "browser: studio hook parameters are baked into the delivered source",
    componentType: "slider",
    evidence: "rendered-pixels",
    expectedObservable:
      "A knob is baked into the assembled source like every other value, so what is delivered is the composition rather than a program that could be one -- a recipient supplies nothing.",
    fixture: SELECTED_LAYER_FIXTURE,
    id: "stack.hookD",
    kind: "control",
    target: "stack.hookD",
    userAction: "Write a chunk that reads uHookD, then move Your parameter D.",
  },
  {
