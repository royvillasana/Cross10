...
/**
 * Timeline acceptance domain: the transport itself.
 *
 * Separate file from `product-motion.spec.ts` because the delivery catalog
 * requires one acceptance domain per spec file, and the line it draws is a real
 * one. This is `timeline.playback` — the runtime's transport, which plays,
 * scrubs, resizes and closes its loop. The motion file is `selectedLayer.*` —
 * the drift rates a layer carries. The two fail differently: a broken transport
 * moves nothing, broken drift moves the wrong thing.
 *
 * The helpers above are shared with the motion file rather than imported from
 * it, because a spec importing another spec makes the second one's tests run
 * twice under Playwright's collection.
 */
